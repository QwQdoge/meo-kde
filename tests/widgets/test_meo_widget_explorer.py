...
class MeoWidgetExplorerTests(unittest.TestCase):
    ...
    def test_performance_manager_is_shared_and_reachable(self):
        ...
        self.assertIn("updatesPaused", manager)
        self.assertIn("MeoSystem.Tasks.paused", manager)
        self.assertIn("MeoSystem.Performance.paused", manager)
        ...
        self.assertIn("virtualMemoryBytes", details_page)
        self.assertIn("peakResidentMemoryBytes", details_page)
        self.assertIn("tcpEstablishedCount", details_page)
        self.assertIn("gpuEngine", details_page)
        self.assertIn("OOM score", details_page)
        self.assertIn("All scopes", services_page)
        self.assertIn("System services are shown read-only", services_page)
        self.assertIn("selectService", services_page)
        self.assertIn("selectedServiceDetails", services_page)
        self.assertIn("Main PID", services_page)
        self.assertIn("CPU time", services_page)
        self.assertIn("setStartupEnabled", startup_page)
        users_page = (MEO_KDE_QML / "UsersPage.qml").read_text(encoding="utf-8")
        self.assertIn("processesRequested", users_page)
        self.assertIn("userFilter", process_table)
        ...
        self.assertIn("socketProtocolSummary", task_backend)
        self.assertIn("peakResidentMemoryBytes", task_backend)
        self.assertIn("gpuEngine", task_backend)
        self.assertIn("m_efficiencyOriginalNice.erase", task_backend)
